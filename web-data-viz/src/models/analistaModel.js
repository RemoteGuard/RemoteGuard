...
function listarPorcentagemRAMPorNotebook(fkNotebook) {
  var instrucaoSql = `SELECT porcentagem_ram, data_captura FROM dados WHERE fkNotebook =  ${fkNotebook} ORDER BY data_captura DESC LIMIT 12;
;`; 
  return database.executar(instrucaoSql);
}
function listarPorcentagemCPUPorNotebook(fkNotebook) {
  var instrucaoSql = `SELECT porcentagem_cpu, data_captura FROM dados WHERE fkNotebook = ${fkNotebook} ORDER BY data_captura DESC LIMIT 12;`; 
  return database.executar(instrucaoSql);
}
