...
def capturarDados():
    global esperarProcesso
    cont = 1
    while True:
       # try:
            if esperarProcesso == False:
            ...
            while True:
                    ...
                    for process in psutil.process_iter():
                        ...
                        if(nome not in listaNomesProcessos and process.status() == 'running'):
                                        ...
                                        conn.commit()
                    esperarProcesso = False
                    time.sleep(inpCapturaProcessos)
...
def home():
    ...
    threadCapturaProcessos = threading.Thread(target=capturarProcessos)
    threadCaptura.start()
    threadCapturaProcessos.start()
...
inpCapturaProcessos = 10
inpCapturaProcessos = 60

def discretizacaoTemporal():
    global inpCapturaProcessos
    global inpCapturaDados
    inpCapturaDados = int(input('Digite de quanto em quanto tempo deseja capturar os dados(em segundos): '))   
    inpCapturaProcessos = int(input('Digite de quanto em quanto tempo deseja capturar os processos(em segundos): '))   
    home()

discretizacaoTemporal()
